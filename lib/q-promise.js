...
P.prototype.then = function then( resolve, reject ) {
    ...
    switch (this.state) {
    case 'y':
        if (typeof resolve === 'function') onNextTick(resolve, this.value, p2, 'y');
        else { p2.state = this.state; p2.value = this.value }
        break;
    case 'n':
        if (typeof reject === 'function') onNextTick(reject, this.value, p2, 'n');
        else { p2.state = this.state; p2.value = this.value }
        break;
    case '':
...
function onNextTick( func, a1, a2, a3 ) {
    ...
    if (!__nextTickScheduled) {
        ...
        __nextTickStack = [ closure ];
    }
    else {
        __nextTickStack.push(closure);
    }
}
function nextTick( closure ) {
    if (!__nextTickScheduled) {
        scheduleNextTick(__runNextTickFunctions);
        __nextTickScheduled = true;
        __nextTickStack = [ closure ];
