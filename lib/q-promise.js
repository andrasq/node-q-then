...
P.promisify = function( fn, Promise ) {
...
function __saveAllResults( state, i, p1, p2 ) {
    var then = _getThenMethod(p1, p2);
    if (then) __resolveGenericThenable(p1, new P(), then, function(v, s) {
        state.results[i] = v;
        state.nresults += 1;
    ...
    else __resolveNo(new Error("not a thenable"), p2);
}
...
function _handleThen( handler, v, p2, state, pBy ) {
    if (typeof handler === 'function') onNextTick(_handleThenHandler, handler, v, p2, pBy);
        ...
        if (_stackDepth <= 10) {
            ...
            onNextTick(__settle, v, p2, state);
            _stackDepth = 0;
        }
    }
}
