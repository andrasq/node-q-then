...
// FIXME: is v allowed to be a function?
        nextTick(__buildNotifyFunc(undefined, v, p));
        return p;
    }
    else
**/
...
P.prototype.then = function then( resolve, reject ) {
    ...
    if (this.state) {
        if (this.state === 'y' && typeof resolve === 'function') {
            nextTick(__buildNotifyFunc(resolve, this.value, p2));
        }
        else if (this.state === 'n' && typeof reject === 'function') {
            nextTick(__buildNotifyFunc(reject, this.value, p2));
        }
        else {
            p2.state = this.state;
            p2.value = this.value;
...
function __resolve( v, p1, state ) {
    ...
    if (v && typeof v === 'object') {
        ...
        if (then) return __resolveThenable(v, p1, state);
    }
...
function __resolveThenable( vt, p1, state ) {
    var self = this;
    if (vt.constructor === P) {
        if (!vt.state) {
...
function __settle( v, p, state ) {
    (state === 'y') ? __settleYes(v, p) : __settleNo(v, p);
}
function __settleYes( v, p ) {
    if (p.state) return;
    p.state = 'y';
    p.value = v;
    if (p.p2s) __settleNotify(v, p, p.yes);
}
function __settleNo( v, p ) {
    if (p.state) return;
    p.state = 'n';
    p.value = v;
    if (p.p2s) __settleNotify(v, p, p.no);
}
function __settleNotify( v, p, funcs ) {
    if (typeof funcs === 'function') {
        // would run 60% faster if not for the double wrappering
        nextTick(__buildNotifyFunc(funcs, v, p.p2s));
    }
    else for (var i=0; i<funcs.length; i++) {
        if (typeof funcs[i] === 'function') {
            nextTick(__buildNotifyFunc(funcs[i], v, p.p2s[i]));
        }
    }
 ...
 */
function __buildNotifyFunc( func, v, p ) {
    return function() {
        try {
            var value = func(v, p);
            __resolve(value, p, 'y');
 ...
 * The functions must never throw (as ensured by __buildNotifyFunc).
 */
var scheduleNextTick = (process.version >= 'v4.') ? process.nextTick : setImmediate;
var __nextTickScheduled = false;
var __nextTicksStack = null;            // new (require('qlist'))();
function nextTick( func ) {
    if (!__nextTickScheduled) {
        scheduleNextTick(__runNextTicks);
        __nextTickScheduled = true;
        __nextTicksStack = [ func ];
    }
    else {
        __nextTicksStack.push(func);
    }
}
function __runNextTicks( ) {
    var calls = __nextTicksStack;
    __nextTicksStack = null;
