...
P.prototype.then = function then( resolve, reject ) {
    ...
    if (this.state) {
        (this.state === 'y') ? _thenResolve(this, this.value, resolve) : _thenReject(this, this.value, reject);
    }
    else {
        var self = this;
        this.yes = _addListener(this.yes, function(){ _thenResolve(self, self.value, resolve) });
        this.no = _addListener(this.no, function(){ _thenReject(self, self.value, reject) });
    }
    return p;
}
