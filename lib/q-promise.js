...
P.race = function race( promises ) {
    ...
    for (var i=0; i<promises.length; i++) {
        ...
        if (p1 && p1.constructor === P) {
            ...
            var called = false;
            var err = _callThenMethod(p2, then, p1,
                function(v) { if (!called) { called = true; __resolveYes(v, p2, p1) } },
                function(e) { if (!called) { called = true; __resolveYes(e, p2, p1) } });
            if (err && !called) __resolveNo(err, p2);
        }
    }
...
function __saveAllResults( state, i, p1, p2 ) {
    ...
    var called = false;
    var err = _callThenMethod(p2, then, p1,
        function(v){ if (!called) { called = true;
            state.results[i] = v; state.nresults += 1;
            if (state.nresults === state.nexpect) __resolveYes(state.results, p2, p2) } },
        function(e){ if (!called) { called = true; __resolveNo(e, p2, p1) } });
    if (err && !called) __resolveNo(err, p2);
}
...
function __resolveGenericThenable( p1, p2, then ) {
    ...
    var err = _callThenMethod(p2, then, p1,
        function(v){ if (!called) { called = true; __resolveYes(v, p2, p1) } },
        function(e){ if (!called) { called = true; __resolveNo(e, p2, p1) } });
    if (err && !called) __settle(err, p2, _REJECTED);
    return  p2;
}
