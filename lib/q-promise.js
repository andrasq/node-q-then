...
P.prototype._yes = null;        // resolve() listeners
P.prototype._no = null;         // reject() listeners
P.prototype._p2s = null;         // other listening P promises pending on this (5% slower by a longer var name??)
...
function __setState( v, p, state ) {
    ...
    if (p._p2s) {
        var _p2s = p._p2s;
        if (state === _RESOLVED) {
            var funcs = p._yes, _p2s = p._p2s;
            p._yes = p._no = p._p2s = null;
            __notifyListenersYes(v, _p2s, funcs);
        } else {
            var _p2s = p._p2s, funcs = p._no;
            p._yes = p._no = p._p2s = null;
            __notifyListenersNo(v, _p2s, funcs);
        }
    }
}
 ...
 * if p1._p2s is not an array, the list is omitted, just the item is stored.
 * invariant: p1._p2s is either a function, an array, or falsy.
 * invariant: if p1._p2s then p1._yes and p1._no are either a function, an array, or falsy.
 */
function __addListeners( p1, p2, resolve, reject ) {
    if (!p1._p2s) {
        p1._p2s = p2;
        p1._yes = typeof resolve === 'function' ? resolve : false;
        p1._no = typeof reject === 'function' ? reject : false;
    }
    else if (p1._p2s.constructor !== Array) {
        p1._p2s = [p1._p2s, p2];
 ...
 * The promise p2, or the array of multipel _p2s, is passed in p._p2s.
 * note: __settleYes is sync, resolve() runs on nextTick, ie might re-order promises
 * invariant: p._p2s is a P or an array of P
...
function __notifyListenersYes( v, _p2s, handlers ) {
    if (typeof handlers === 'function') {
        onNextTick(__resolveThenYes, handlers, v, _p2s);
    }
    else if (!handlers) {
        __settleYes(v, _p2s);
    }
        ...
        for (var i=0; i<handlers.length; i++) {
            if (typeof handlers[i] === 'function') onNextTick(__resolveThenYes, handlers[i], v, _p2s[i]);
            else __settleYes(v, _p2s[i]);
        }
    }
}
function __notifyListenersNo( v, _p2s, handlers ) {
    if (typeof handlers === 'function') {
        onNextTick(__resolveThenNo, handlers, v, _p2s);
    }
    else if (!handlers) {
        __settleNo(v, _p2s);
    }
        ...
        for (var i=0; i<handlers.length; i++) {
            if (typeof handlers[i] === 'function') onNextTick(__resolveThenNo, handlers[i], v, _p2s[i]);
            else __settleNo(v, _p2s[i]);
        }
    }
}
