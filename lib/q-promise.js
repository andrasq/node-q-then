...
function P( resolver ) {
    this.state = _PENDING;      // '' pending, 'y' fulfilled, 'n' rejected
...
P.race = function race( promises ) {
    ...
    for (var i=0; i<promises.length; i++) {
        ...
        then.call(p1,
            function(v) { __resolveYes(v, p2) },
            function(e) { __resolveNo(e, p2) }
        );
    }
...
// If any reject, reject with that cause.  If promises is empty array, fufill with [].
P.all = function all( promises ) {
    var p2 = new P();

    if (!promises.length) return __resolveYes([], p2);
    ...
    for (var i=0; i<promises.length; i++) {
        __saveAllResults(state, promises, i, p2);
        if (p2.state) return p2;
    }

    return p2;
}
function __saveAllResult( state, promises, i, p2 ) {
    var p1 = promises[i];
    var then = _getThenMethod(p1, p2);
    if (!then || p2.state) { __settleNo(new Error("not a thenable"), p2); return p2 }
    ...
    then.call(p1,
        function(v) {
            state.results[i] = v;
            state.nresults += 1;
            if (state.nresults === promises.length) __resolveYes(results, p2)
        },
        function(e) {
            __resolveNo(e, p2)
        }
...
function _tryExecutor( p1, resolver ) {
    try {
        resolver(function(v) { __resolveYes(v, p1) }, function(e) { __resolveNo(e, p1) })
    }
    catch (e) {
        __resolveNo(e, p1);
    }
...
function __resolveThenable( vt, p2, state, then ) {
    if (vt === p2) {
        ...
        if (!vt.state) {
// FIXME: should a promise be able to return another promise to resolveHandler?  recursively forever?
            __addListeners(vt, p2, __resolveYes, __resolveNo);
        } else {
...
function _callThenMethod( p, thenMethod, thenable, resolveHandler, rejectHandler ) {
