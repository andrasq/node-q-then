...
P.prototype.then = function then( resolve, reject ) {
    ...
    switch (this.state) {
    case _RESOLVED:
        if (typeof resolve === 'function') _resolveThenYes(resolve, this.value, p2);
        else { p2.state = this.state; p2.value = this.value }
        break;
    case _REJECTED:
        if (reject && typeof reject === 'function') _resolveThenNo(reject, this.value, p2);
        else { p2.state = this.state; p2.value = this.value }
        break;
    case _PENDING:
...
function __resolveYes( v, p1 ) {
    if (_onResolveCb) _onResolveCb(v, p1, _RESOLVED);
    if (!p1.state) {
        if ((typeof v === 'object' && v) || typeof v === 'function') {
            if (v.constructor === P) return __resolveThenable(v, p1);
            if (__resolveMaybeThenable(v, p1)) return p1;
        }
        __setState(v, p1, _RESOLVED);
    }
    return p1;
}
function __resolveNo( v, p1 ) {
    if (_onResolveCb) _onResolveCb(v, p1, _REJECTED);
    if (!p1.state) {
        if ((typeof v === 'object' && v) || typeof v === 'function') {
            if (v.constructor === P) return __resolveThenable(v, p1);
            if (__resolveMaybeThenable(v, p1)) return p1;
        }
        __setState(v, p1, _REJECTED);
    }
    return p1;
...
function __resolveMaybeThenable( v, p1 ) {
    var then = _getThenMethod(v, p1);
    if (then) return __resolveThenable(v, p1, _RESOLVED, then);
...
function __resolveThenable( vt, p2, state, then ) {
    if (vt === p2) {
        ...
        if (!vt.state) {
            _addThenListener(vt, p2, null, null);
        } else {
 ...
 * The alternate of a single queue of listener objects { p2, resolve, reject } is 8% slower.
 */
...
function _notifyThenListeners( v, p1, state ) {
    ...
    if (listeners.constructor !== Array) {
        state === _RESOLVED ? _resolveThenYes(p1._yes, v, listeners) : _resolveThenNo(p1._no, v, listeners);
        ...
        if (state === _RESOLVED) {
            for (var i=0; i<len; i++) _resolveThenYes(handlers[i], v, listeners[i]);
        }
        else {
            for (var i=0; i<len; i++) _resolveThenNo(handlers[i], v, listeners[i]);
        }
        p1._listeners = p1._yes = p1._no = null;
    }

}
function _resolveThenYes( resolve, v, p2 ) {
    if (typeof resolve === 'function') onNextTick(_resolveThenHandler, resolve, v, p2);
    else __settle(v, p2, _RESOLVED);
}
function _resolveThenNo( reject, v, p2 ) {
    if (typeof reject === 'function') onNextTick(_resolveThenHandler, reject, v, p2);
    else __settle(v, p2, _REJECTED);
}
...
// hence the onNextTick() wrappering in _resolveThenYes/No.
function _resolveThenHandler( handler, v, p2 ) {
