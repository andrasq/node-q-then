...
P.prototype.then = function then( resolve, reject ) {
    ...
    case 'y':
        ...
        break;
    case 'n':
        if (typeof reject === 'function') onNextTick(__resolveThenNo, this.value, p2);
        else { p2.state = this.state; p2.value = this.value }
        break;
    case '':
 ...
 * invariants: p.p2s != null, handlers is one of {function, falsy, array of any}
 */
function __notifyListeners( v, p, funcs, settle, state ) {
    if (p.p2s.constructor === P) {
        // benchmark would run 60% faster if not for the double wrappering
        if (typeof funcs === 'function') onNextTick(funcs, v, p.p2s, state);
        else onNextTick(settle, v, p.p2s, state);
    }
    else for (var i=0; i<funcs.length; i++) {
        if (typeof funcs[i] === 'function') onNextTick(funcs[i], v, p.p2s[i], state);
        else onNextTick(settle, v, p.p2s[i], state);
    }
    p.yes = p.no = p.p2s = null;
}
...
function __resolveThenYes( resolveHandler, v, p2 ) {
    try {
        var ret = resolveHandler(v);
        (ret !== undefined) ? __resolve(ret, p2, 'y') : __resolve(v, p2, 'y');
    }
    catch (err) {
        __resolve(err, p2, 'n');
    }
...
function __resolveThenNo( rejectHandler, v, p2 ) {
    try {
        var ret = rejectHandler(v);
        (ret !== undefined) ? __resolve(ret, p2, 'y') : __resolve(v, p2, 'n');
    }
    catch (err) {
        __resolve(err, p2, 'n');
    }
 ...
 */
function onNextTick( func, a1, a2, a3 ) {
    //var closure = [fn, a1, a2, a3];
    var closure = {f: func, v: a1, p: a2, s: a3};
...
function runTickQueue( ) {
    ...
    for (var i=0; i<closures.length; i++) {
        ...
        //closures[i][0](closures[i][1], closures[i][2]);
        __invokeNextTickClosure(closures[i].f, closures[i].v, closures[i].p, closures[i].s);
    }
...
function __invokeNextTickClosure( f, v, p ) {
    try {
        var value = f(v, p);
        if (value !== undefined) __resolve(value, p, 'y');
        else __resolve(v, p, state);
    }
    catch (err) {
...
function _getThenMethod( vt, p1 ) {
    ...
    catch (e) { if (p1) __settleNo(e, p1); return false }
}
